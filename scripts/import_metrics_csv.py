from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from app.database import SessionLocal, init_db
from app.metrics_intake import CSVImporter, MetricsIntakeError


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import metrics intake CSV.")
    parser.add_argument("--csv", required=True)
    parser.add_argument("--source-id", type=int)
    parser.add_argument("--campaign-id", type=int)
    parser.add_argument("--source-type", default="manual_csv")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    init_db()
    csv_path = Path(args.csv)
    try:
        with SessionLocal() as db:
            result = CSVImporter(db).import_csv_text(
                csv_path.read_text(encoding="utf-8-sig"),
                source_id=args.source_id,
                campaign_id=args.campaign_id,
                source_type=args.source_type,
                source_name=str(csv_path),
            )
    except (MetricsIntakeError, OSError) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 2
    print(json.dumps(result.model_dump(mode="json"), indent=2, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

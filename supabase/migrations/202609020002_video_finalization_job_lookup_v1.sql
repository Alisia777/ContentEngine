begin;
-- 202609020002_video_finalization_job_lookup_v1
--
-- Архивная проекция стратегий не отдаёт браузеру output_media_id готового
-- ролика — у карточки запуска есть только generation_job_id. Готовый файл
-- при этом привязан метадатой: media_objects.metadata->>'generation_job_id'.
-- Постановка финализации принимает РОВНО ОДИН из ключей: media_id (прямой
-- путь) или generation_job_id (поиск готового ролика по наряду генерации).
-- Функция пере-эмитится целиком (202609020001 уже применена и по правилам
-- деплоя не редактируется).
...
begin
  ...
  if position('video_finalization_media_reference_invalid'
       in definition_value) = 0 then
    raise exception using message =
      'video_finalization_reference_guard_missing';
  end if;
  if not exists (
    select 1

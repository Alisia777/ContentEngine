begin;
-- 202608260006_content_passport_media_links_v1
--
-- «Тут бы фото добавить... и ссылку на ролик или плеер» (владелец, 26.08
-- 21:20): паспорту нужны сами медиа. Оба RPC отдают object_name приватного
-- хранилища (ролик реестра, ролик паспорта, каждый материал запуска) —
-- браузер получает подписанные ссылки существующим батчем
-- signedPrivateObjectUrls и рисует плеер и миниатюры. Полные переопределения:
-- registry — из 202608260003 + volatile (урок 202608260004) + object_name;
-- passport — из 202608260005 + object_name медиа и материалов.
...
begin
  ...
  if strategy_row.id is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'object_name', asset_media.object_name,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'ai_watched', reference.ai_watched,
      'registered_at', reference.applied_at
    ) order by binding.id), '[]'::jsonb)
    into sources_value
  from content_factory.generation_job_video_reference_bindings binding
  left join content_factory.generation_spec_video_reference_bindings reference
    on reference.organization_id = binding.organization_id
    and reference.id = binding.spec_reference_binding_id

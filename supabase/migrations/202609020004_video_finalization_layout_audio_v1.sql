begin;
-- 202609020004_video_finalization_layout_audio_v1
--
-- Третья итерация «Финализации»: позиция каждой плашки (top|bottom),
-- размер шрифта (small|medium|large) и режим звука (replace — родная
-- дорожка глушится, duck — тихо остаётся под голосом). Все три ключа
-- опциональны; словари сквозные с воркером и select'ами диалога.
-- Функция пере-эмитится целиком (0001-0003 применены и не редактируются).
...
declare
  ...
  position_item jsonb;
  font_scale_value text;
  audio_mode_value text;
  job_id_value uuid;
  params_value jsonb;
...
begin
  ...
  -- Позиции плашек: либо ключа нет (верх/низ/низ), либо три значения.
  if p_payload ? 'caption_positions' then
    positions_value := p_payload -> 'caption_positions';
    if jsonb_typeof(positions_value) <> 'array'
       or jsonb_array_length(positions_value) <> 3 then
      raise exception using errcode = '22023',
        message = 'video_finalization_caption_positions_invalid';
    end if;
    for position_item in
      select value from jsonb_array_elements(positions_value)
    loop
      if jsonb_typeof(position_item) <> 'string'
         or position_item #>> '{}' not in ('top', 'bottom') then
        raise exception using errcode = '22023',
          message = 'video_finalization_caption_positions_invalid';
      end if;
    end loop;
  ...
  font_scale_value := nullif(coalesce(p_payload ->> 'font_scale', ''), '');
  if font_scale_value is not null
     and font_scale_value not in ('small', 'medium', 'large') then
    raise exception using errcode = '22023',
      message = 'video_finalization_font_scale_invalid';
  end if;
  audio_mode_value := nullif(coalesce(p_payload ->> 'audio_mode', ''), '');
  if audio_mode_value is not null
     and audio_mode_value not in ('replace', 'duck') then
    raise exception using errcode = '22023',
      message = 'video_finalization_audio_mode_invalid';
  end if;
  ...
  if job_row.id is not null then
    ...
    params_value := jsonb_strip_nulls(jsonb_build_object(
      ...
      'caption_positions', positions_value,
      'font_scale', font_scale_value,
      'audio_mode', audio_mode_value,
      'output_object_name',
...
begin
  ...
  if position('video_finalization_caption_positions_invalid'
       in definition_value) = 0
     or position('video_finalization_font_scale_invalid'
       in definition_value) = 0
     or position('video_finalization_audio_mode_invalid'
       in definition_value) = 0 then
    raise exception using message =
      'video_finalization_layout_guards_missing';
  end if;

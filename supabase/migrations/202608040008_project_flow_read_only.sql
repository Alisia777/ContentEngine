...
-- The project catalog is a read-only RPC.  Do not refresh the profile row here:
-- every authenticated workspace request already passes through creator_bootstrap,
-- and an unconditional profile upsert serializes otherwise independent reads on
-- the same profile row.  CREATE OR REPLACE keeps the installed function identity,
-- owner and dependent PostgREST contract intact.
...
comment on function public.creator_project_flow(jsonb) is
  'Returns one exact selected-project flow plus a lightweight read-only project chooser catalog.';

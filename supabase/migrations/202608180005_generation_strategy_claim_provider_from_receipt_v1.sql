...
-- 202608180005_generation_strategy_claim_provider_from_receipt_v1
--
-- Клейм платного старта перестаёт вписывать провайдера словом.
--
-- Раньше функция пять раз писала 'runway' в наряд, батч, задачу и карточку
-- проверки. Из-за этого любой второй движок оказался бы записан как Runway:
-- деньги списались бы у одного провайдера, а учёт указывал бы на другого.
--
-- Теперь провайдер берётся из квитанции готовности — того же источника, что и
-- цена. Он подписан вместе с ней, поэтому подменить движок после расчёта
-- невозможно. Тело функции в остальном повторяет 202608180001 без изменений.
...
begin
  ...
  insert into content_factory.generation_batches (
    ...
    'strategy-batch:' || claim_hash_value, receipt_row.provider,
    receipt_row.recipe,
  ...
  insert into content_factory.generation_jobs (
    ...
    actor_id_value, actor_id_value, 'real', receipt_row.provider, true,
...
-- Батчи и задачи должны принимать второй движок, иначе клейм упадёт на
-- проверке сразу после того, как деньги уже зарезервированы.
...
alter table content_factory.generation_batches
  ...
  check (
    mode <> 'real'
    or provider is null
    or (provider = 'runway' and model = any (array[
      'gen4_turbo', 'seedance2_fast', 'seedream5_lite',
      'product_ugc', 'product_swap', 'product_ad'
    ]))
    or (provider = 'google' and model = 'veo-3.1-lite-generate-preview')
    or (provider = 'fal' and model = any (array[
      'product_ugc', 'product_swap', 'product_ad'
    ]))
  );

do $claim_provider_from_receipt_verify$
declare
...
begin
  ...
  where n.nspname = 'public'
    and p.proname = 'system_claim_generation_strategy_start';
  if definition_value is null
     or position('''runway''' in definition_value) > 0
     or position('receipt_row.provider' in definition_value) = 0
     or position('job.status <> ''failed''' in definition_value) = 0 then
    raise exception using message = 'claim_provider_from_receipt_verify_failed';
  end if;
end;
$claim_provider_from_receipt_verify$;

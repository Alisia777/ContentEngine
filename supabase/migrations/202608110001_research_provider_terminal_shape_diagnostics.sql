...
-- Migration 015 is already immutable in production. This future-only layer
-- preserves its receipts and broadens the bounded app-owned shape codes written
-- by creator-product-research after this migration. The writer also retains the
-- exact bounded v2 code/type allowlists while the old Edge bundle can still be
-- live between this migration and the later function-deploy step. The new Edge
-- bundle never emits those legacy types or the generic failed-shape code.
alter table content_factory.research_provider_health_receipts
  drop constraint if exists
    research_provider_health_receipts_provider_error_code_check;

alter table content_factory.research_provider_health_receipts
  add constraint research_provider_health_error_code_v2_check
  check (
...
alter table content_factory.research_provider_health_receipts
  validate constraint research_provider_health_error_code_v2_check;
...
alter table content_factory.research_provider_health_receipts
  add constraint research_provider_health_terminal_shape_v2_consistent
  check (
    (
...
alter table content_factory.research_provider_health_receipts
  validate constraint research_provider_health_terminal_shape_v2_consistent;

-- Replace only the passive-health writer. Request authorization, paid POST

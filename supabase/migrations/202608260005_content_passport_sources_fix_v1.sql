begin;
-- 202608260005_content_passport_sources_fix_v1
--
-- Боевой отказ 26.08 21:13: «Открыть паспорт» падал с 42703 — в
-- generation_job_video_reference_bindings нет колонки source_id (я предположил
-- её вслепую). Привязка наряда ссылается на spec-привязку референса
-- (spec_reference_binding_id), и именно там живут canonical_url, платформа,
-- mechanics_summary и отметка «ИИ ролик не просматривал». Полное
-- переопределение функции с исправленным блоком sources; volatile — по уроку
-- 202608260004 (current_profile_id пишет профиль при каждом вызове).

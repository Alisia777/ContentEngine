...
Облачный кабинет поддерживает два контура:

- бесплатные mock-пакеты до 50 задач для обучения и проверки процесса;
- поштучную spend-gated генерацию через Runway: квадратное фото 2K
  `seedream5_lite`, видео 5 секунд `gen4_turbo` и вертикальное видео 8 секунд
  со звуком `seedance2_fast`.

Перед каждым реальным запуском портал требует проверенный исходник и права,
активную кампанию с денежным лимитом, автоматически собирает безопасное ТЗ,
дожидается бесплатной проверки обученной политики, проверяет доступность модели,
кредиты и дневную квоту, а затем просит отдельное подтверждение точной цены.
Задача, результат и стоимость сверяются фоновым worker, готовый файл сохраняется
в приватное Storage и перед публикацией проходит контент-проверку.

Контур реального провайдера и бесплатная readiness-проверка внедрены. Полная
production-приёмка каждого платного SKU завершается только отдельным минимальным
smoke-запуском после явного подтверждения его цены; тесты и readiness-проверки
сами по себе ничего не списывают. Python/FastAPI, SQLite и Dockerfile сохранены
как reference/regression-контур, а не как production deploy.

Инструкция запуска, обязательные переменные и release checklist:
[docs/CLOUD_DEPLOYMENT.md](docs/CLOUD_DEPLOYMENT.md).
Текущее доказательство готовности и оставшиеся платные smoke:
[docs/PRODUCTION_GENERATION_ACCEPTANCE.md](docs/PRODUCTION_GENERATION_ACCEPTANCE.md).

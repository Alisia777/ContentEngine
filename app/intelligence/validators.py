...
def validate_script_claim_refs(script: GeneratedScriptOutput, brief: ScriptBriefOutput) -> dict:
    ...
    allowed_pairs = {(claim.source_type, claim.source_key) for claim in brief.allowed_claims}
    errors: list[str] = []
    for scene in script.scenes:
        for ref in scene.claim_refs:
            normalized = ref.removeprefix("product.")
            if normalized in allowed_refs:
                continue
            if ":" in normalized:
                source_type, source_key = normalized.split(":", 1)
                if (source_type, source_key) in allowed_pairs:
                    continue
            else:

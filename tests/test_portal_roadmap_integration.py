...
def test_account_launch_center_is_reachable_from_the_live_portal() -> None:
    assert 'from "./account-launch-view.js?v=20260826.rebuild-clean.45"' in APP
    assert 'from "./account-launch-guides.js?v=20260826.rebuild-clean.45"' in (
    ...
    assert './account-launch.css?v=20260826.rebuild-clean.45' in INDEX
    assert 'const ACCOUNT_VISUAL_MODULE_URL = "./account-launch-visual-examples.js?v=20260826.rebuild-clean.45"' in APP
    ...
    assert './account-launch-visual-examples.css?v=20260826.rebuild-clean.45' in INDEX
    assert './first-shift-full-scenario.css?v=20260826.rebuild-clean.45' in INDEX
...
def test_manager_dashboard_uses_the_scoped_rpc_and_live_handlers() -> None:
    ...
    assert 'from "./manager-dashboard-view.js?v=20260826.rebuild-clean.45"' in APP
    assert 'from "./access-center-view.js?v=20260826.rebuild-clean.45"' in APP
    ...
    assert './manager-dashboard.css?v=20260826.rebuild-clean.45' in INDEX
...
def test_release_entry_assets_use_current_cache_versions() -> None:
    assert './styles.css?v=20260826.rebuild-clean.45' in INDEX
    assert './config.js?v=20260826.rebuild-clean.45' in INDEX
    assert f'<meta name="contentengine-build" content="{BUILD_ID}"' in INDEX
    assert './app.js?v=20260826.rebuild-clean.45' in INDEX
    assert './workspace-os-v4-loader.js?v=20260826.rebuild-clean.45' in INDEX
    assert f'./interface-system.css?v={BUILD_ID}' in INDEX
    assert './workspace-os-v4.css?v=20260826.rebuild-clean.45' in INDEX
    assert f'./workspace-build-guard.js?v={BUILD_ID}' in INDEX
    assert './supabase-api.js?v=20260826.rebuild-clean.45' in APP
    assert './catalog.js?v=20260826.rebuild-clean.45' in APP

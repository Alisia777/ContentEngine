...
def test_strategy_harness_is_server_catalog_driven_and_uses_the_portal_form() -> None:
    assert (
        'from "./generation-strategy-view.js?v=20260826.rebuild-clean.30"' in SUBJECT
    )
    assert (
        'from "./generation-strategy-assets.js?v=20260826.rebuild-clean.30"' in SUBJECT
    )
...
def test_strategy_engine_choice_is_step_two_above_the_fold() -> None:
    ...
    # Порядок в main: товар → каскад → подсказка → комментарий → гипотеза →
    # кампания. Гипотеза запуска стоит после замысла (26.08, контур №3).

...
def test_v49_loader_script_adapters_and_one_shared_operations_style() -> None:
    assert 'const BUILD = "20260826.rebuild-clean.58"' in LOADER
...
def test_same_route_dom_patch_preserves_live_surfaces_and_stable_records() -> None:
    assert 'import { patchWorkspaceContent } from "./workspace-dom-patch.js?v=20260826.rebuild-clean.58"' in APP_JS
    for marker in (

...
FROZEN_AUTHORITY_HASHES = {
    ...
    # Отпечаток сдвинут осознанно 19.08.2026: у «Копии» появился второй и
    # третий движок, и ступени кредитов Runway перестали быть ответом
    # generation_strategy_recipe_price — та отвечает про ДЕЙСТВУЮЩИЙ маршрут
    # (202608180006/202608180010). Проверка ступеней переехала на маршрутную
    # функцию, а у recipe_price проверяется её собственный инвариант: ответ
    # обязан совпасть с ценой действующей строки реестра. Ни одно число тарифа
    # при этом не изменилось.
    # Отпечаток сдвинут осознанно 22.08.2026: «Аватар» переопределён в «Дуэт»,
    # и кадр ему задаёт исходник, а не выбор соотношения сторон — миграция
    # 202608220014 требует ratio 'source'. Два утверждения о ступенях тарифа
    # звали цену с отменёнными вертикалями 720:1280 и 1080:1920 и получали
    # NULL. Числа тарифа (192 и 648) при этом не изменились ни одно: правится
    # проверка кадра, а не арифметика. Добавлено третье утверждение — что
    # отменённое измерение теперь ОТВЕРГАЕТСЯ, иначе возврат старой формы
    # прошёл бы молча.
    # Отпечаток сдвинут осознанно 23.08.2026: утверждение о полномочии политики
    # провайдера пинило ЦЕЛИКОМ строку `launch_enabled_value := binding_...`,
    # а миграция 202608230011 обернула вычисление в coalesce(..., false) и
    # добавила слагаемое — признак исполнимого маршрута. Набор обязательных
    # условий не убавился, а прибавился, поэтому пин переехал на сами
    # слагаемые: отступы вокруг них ничего не стерегли.
    # Отпечаток сдвинут осознанно 23.08.2026 (движки): «Создание» и «Дуэт»
    # больше не считаются рунвеевскими ступенями — у рецептов product_ad и
    # product_ugc действующие маршруты fal и HeyGen с посекундной ставкой, а
    # 1080p для движков с единственным режимом 720p отвергается, а не
    # оценивается молча. Числа ступеней «Копии» не тронуты.
    AUTHORITY_PGTAP: (
        "263c7bc864d495d994045029272b5dcfb08c395700d71a395c1a0a91cc931faf"
    ),
    AUTHORITY_STATIC: (

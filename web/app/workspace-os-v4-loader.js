/*
 * ContentEngine Desktop v4.17 route loader.
 *
...
import { workspaceActionKey } from "./workspace-action-key.js?v=20260804.os4.17";
...
const ROUTE_ASSETS = Object.freeze({
  aiLearning: Object.freeze({
    ...
    styles: ["ai-learning-control-room.css?v=20260804.os4.17"],
    modules: [],
  }),
...
function retry() {
  ...
  retryPromise = loadRoute(route, actionKey)
    .catch((error) => {
      ...
      console.error("ContentEngine Desktop v4.17 route retry failed", error);
      return false;
    })
    .finally(() => {
      retryPromise = null;
...
function loadCurrentRoute() {
  ...
  return loadRoute(route, actionKey).catch((error) => {
    ...
    console.error("ContentEngine Desktop v4.17 current route failed to load", error);
    return false;
  });
}
...
function schedule() {
  ...
  window.queueMicrotask(() => {
    ...
    void loadRoute(scheduledRoute, scheduledActionKey).catch((error) => {
      ...
      console.error("ContentEngine Desktop v4.17 route failed to start", error);
    });
  });
}

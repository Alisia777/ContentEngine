/*
 * ContentEngine Desktop v4.35 route loader.
 *
...
import { workspaceActionKey } from "./workspace-action-key.js?v=20260812.os4.35";
...
function retry() {
  ...
  retryPromise = loadRoute(route, actionKey)
    .catch((error) => {
      ...
      console.error("ContentEngine Desktop v4.35 route retry failed", error);
      return false;
    })
    .finally(() => {
      retryPromise = null;
...
function loadCurrentRoute() {
  ...
  return loadRoute(route, actionKey).catch((error) => {
    ...
    console.error("ContentEngine Desktop v4.35 current route failed to load", error);
    return false;
  });
}
...
function schedule() {
  ...
  window.queueMicrotask(() => {
    ...
    void loadRoute(scheduledRoute, scheduledActionKey).catch((error) => {
      ...
      console.error("ContentEngine Desktop v4.35 route failed to start", error);
    });
  });
}
